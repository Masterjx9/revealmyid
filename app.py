...
def login():
    ...
    if platform == "slack":
        ...
        slack_authorization_url = (
            ...
            f"scope=openid%20email%20profile%20identity.basic&"
...
def slack_callback():
    ...
    token = token_response.json()["authed_user"]["access_token"]
    
    if not token:
        return f"Failed to obtain access token. {token_response.text}"
